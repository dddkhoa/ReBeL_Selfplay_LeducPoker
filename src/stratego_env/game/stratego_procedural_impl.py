...
import numpy as np
from numba import boolean, float32, int64, jit, types
...
@jit(
    INT_DTYPE_JIT(INT_DTYPE_JIT, INT_DTYPE_JIT),
    nopython=True,
    fastmath=True,
    cache=True,
)
def _get_max_possible_actions_per_start_position(
    rows: INT_DTYPE_NP, columns: INT_DTYPE_NP
):
...
@jit(
    INT_DTYPE_JIT[:, :, :](
        INT_DTYPE_JIT[:, :], INT_DTYPE_JIT[:, :], INT_DTYPE_JIT[:, :], INT_DTYPE_JIT
    ),
    nopython=True,
    fastmath=True,
    cache=True,
...
def _create_initial_state(
    ...
    state[StateLayers.PLAYER_2_PIECES.value] = player_2_initial_piece_map[
        ::-1, ::-1
    ].copy()

    # all piece locations are marked as UNKNOWN
    player_1_po_pieces = player_1_initial_piece_map.copy()
    player_1_po_pieces = np.where(
        player_1_po_pieces != SP.NOPIECE.value, SP.UNKNOWN.value, player_1_po_pieces
    )
    state[StateLayers.PLAYER_1_PO_PIECES.value] = player_1_po_pieces

    # all piece locations are marked as UNKNOWN
    player_2_po_pieces = player_2_initial_piece_map[::-1, ::-1].copy()
    player_2_po_pieces = np.where(
        player_2_po_pieces != SP.NOPIECE.value, SP.UNKNOWN.value, player_2_po_pieces
    )
    ...
    player_1_still_pieces = np.where(
        player_1_still_pieces != SP.NOPIECE.value,
        StillPieces.COULD_BE_FLAG.value,
    ...
    player_2_still_pieces = np.where(
        player_2_still_pieces != SP.NOPIECE.value,
        StillPieces.COULD_BE_FLAG.value,
...
def _get_action_size(rows: INT_DTYPE_NP, columns: INT_DTYPE_NP):
    return (
        rows * columns * _get_max_possible_actions_per_start_position(rows, columns)
    ) + 1


@jit(
    types.UniTuple(INT_DTYPE_JIT, 3)(INT_DTYPE_JIT, INT_DTYPE_JIT),
    nopython=True,
    fastmath=True,
    cache=True,
)
...
def _get_action_1d_index_from_positions(
    rows: INT_DTYPE_NP,
    columns: INT_DTYPE_NP,
    start_r: INT_DTYPE_NP,
    ...
    index = (
        ((start_r * columns) + start_c) * max_possible_actions_per_start_position
    ) + end_position_offset
...
@jit(
    types.UniTuple(INT_DTYPE_JIT, 3)(
        INT_DTYPE_JIT,
        INT_DTYPE_JIT,
        INT_DTYPE_JIT,
        INT_DTYPE_JIT,
...
def _get_action_spatial_index_from_positions(
    rows: INT_DTYPE_NP,
    columns: INT_DTYPE_NP,
    start_r: INT_DTYPE_NP,
    start_c: INT_DTYPE_NP,
    ...
    if row_dist > 0:
        ...
        channel_offset = (
            2 * max_row_move_distance_on_board + max_col_move_distance_on_board
        )
    else:
...
@jit(
    types.UniTuple(INT_DTYPE_JIT, 4)(
        INT_DTYPE_JIT, INT_DTYPE_JIT, types.UniTuple(INT_DTYPE_JIT, 3)
    ),
    nopython=True,
    fastmath=True,
    cache=True,
)
def _get_action_positions_from_spatial_index(
    rows: INT_DTYPE_NP, columns: INT_DTYPE_NP, spatial_index
):
...
@jit(
    INT_DTYPE_JIT(
        INT_DTYPE_JIT, INT_DTYPE_JIT, types.UniTuple(INT_DTYPE_JIT, 3), INT_DTYPE_JIT
    ),
    nopython=True,
    fastmath=True,
    cache=True,
)
def _get_action_1d_index_from_spatial_index(
    rows: INT_DTYPE_NP,
    columns: INT_DTYPE_NP,
    spatial_index: Tuple[INT_DTYPE_NP],
    max_possible_actions_per_start_position: INT_DTYPE_NP,
):
    start_r, start_c, end_r, end_c = _get_action_positions_from_spatial_index(
        rows=rows,
        columns=columns,
...
@jit(
    types.UniTuple(INT_DTYPE_JIT, 4)(
        INT_DTYPE_JIT, INT_DTYPE_JIT, INT_DTYPE_JIT, INT_DTYPE_JIT, INT_DTYPE_JIT
    ),
    nopython=True,
    fastmath=True,
    cache=True,
)
def _get_action_positions_from_1d_index(
    rows: INT_DTYPE_NP,
    columns: INT_DTYPE_NP,
    action_index: INT_DTYPE_NP,
    ...
    if action_index == action_size - 1:
        ...
        print(columns)
        raise ValueError(
            "Action is a no-op so it doesn't translate to an actual action"
        )
...
def _get_action_spatial_index_from_1d_index(
    rows: INT_DTYPE_NP,
    columns: INT_DTYPE_NP,
    action_index: INT_DTYPE_NP,
    action_size: INT_DTYPE_NP,
...
def _get_valid_moves_as_spatial_mask(state: np.ndarray, player: INT_DTYPE_NP):
    ...
    spatial_representation_channels = (
        (rows - 1) * 2 + (columns - 1) * 2 + 1
    )  # + 1 for noop action at [0, 0, extra_noop_channel_at_end]

    valid_moves_mask = np.zeros(
        shape=(rows, columns, spatial_representation_channels), dtype=INT_DTYPE_NP
    )
    ...
    if not state[StateData.GAME_OVER.value]:
        for start_r in range(rows):
            for start_c in range(columns):
                ...
                if (
                    piece_type == 0
                    or piece_type == SP.FLAG.value
                    or piece_type == SP.BOMB.value
                ):
                    continue
                    ...
                    for r_dir in row_directions:
                        ...
                        while True:
                            ...
                            if (
                                recent_moves[start_r, start_c]
                                == RecentMoves.JUST_ARRIVED_AND_CANT_DOUBLE_BACK.value
                                and recent_moves[end_r, end_c]
                                == RecentMoves.JUST_CAME_FROM.value
                                and enemy_pieces[end_r, end_c] == 0
                            ):
...
def _get_valid_moves_as_1d_mask(
    state: np.ndarray,
    player: INT_DTYPE_NP,
    action_size: INT_DTYPE_NP,
    max_possible_actions_per_start_position: INT_DTYPE_NP,
...
def _get_state_from_player_perspective(state: np.ndarray, player: INT_DTYPE_NP):
    if player == 1:
        ...
        flipped_state[StateLayers.PLAYER_1_PIECES.value] = state[
            StateLayers.PLAYER_2_PIECES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_2_PIECES.value] = state[
            StateLayers.PLAYER_1_PIECES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.OBSTACLES.value] = state[
            StateLayers.OBSTACLES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_1_PO_PIECES.value] = state[
            StateLayers.PLAYER_2_PO_PIECES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_2_PO_PIECES.value] = state[
            StateLayers.PLAYER_1_PO_PIECES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_1_RECENT_MOVES.value] = state[
            StateLayers.PLAYER_2_RECENT_MOVES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_2_RECENT_MOVES.value] = state[
            StateLayers.PLAYER_1_RECENT_MOVES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_1_STILL_PIECES.value] = state[
            StateLayers.PLAYER_2_STILL_PIECES.value, ::-1, ::-1
        ]
        flipped_state[StateLayers.PLAYER_2_STILL_PIECES.value] = state[
            StateLayers.PLAYER_1_STILL_PIECES.value, ::-1, ::-1
        ]
        ...
        flipped_state[p1_cap_start:p1_cap_end] = state[
            p2_cap_start:p2_cap_end, ::-1, ::-1
        ]
        flipped_state[p2_cap_start:p2_cap_end] = state[
            p1_cap_start:p1_cap_end, ::-1, ::-1
        ]
...
def _get_action_positions_from_player_perspective(
    player: INT_DTYPE_NP,
    start_r: INT_DTYPE_NP,
    start_c: INT_DTYPE_NP,
    end_r: INT_DTYPE_NP,
...
def _get_action_1d_index_from_player_perspective(
    action_index,
    player,
    action_size,
    rows,
    ...
    if player == 1:
        ...
        (
            ...
            flipped_end_r,
            flipped_end_c,
        ) = _get_action_positions_from_player_perspective(
            player=player,
            start_r=start_r,
            start_c=start_c,
            end_r=end_r,
        ...
        return _get_action_1d_index_from_positions(
            rows=rows,
            columns=columns,
            start_r=flipped_start_r,
            start_c=flipped_start_c,
            end_r=flipped_end_r,
            end_c=flipped_end_c,
            max_possible_actions_per_start_position=max_possible_actions_per_start_position,
        )
...
def _is_move_valid_by_position(
    state: np.ndarray,
    player: INT_DTYPE_NP,
    start_r: INT_DTYPE_NP,
    start_c: INT_DTYPE_NP,
    ...
    if (
        ...
        or start_r < 0
        or start_r >= rows
        or obstacles[start_r, start_c] != 0
    ):
    ...
    if (
        moved_piece_type == 0
        or moved_piece_type == SP.FLAG.value
        or moved_piece_type == SP.BOMB.value
    ):
    ...
    if (
        recent_moves[start_r, start_c]
        == RecentMoves.JUST_ARRIVED_AND_CANT_DOUBLE_BACK.value
        and recent_moves[end_r, end_c] == RecentMoves.JUST_CAME_FROM.value
        and enemy_pieces[end_r, end_c] == 0
        and (not allow_piece_oscillation)
    ):
        # piece can't double back here due to rules about oscillating in the same place
        print(
            "piece can't double back here due to rules about oscillating in the same place"
    ...
    if moved_piece_type == SP.SCOUT.value:
        ...
        if end_r != start_r:
            ...
            for intermediate_r in range(
                start_r + vertical_direction, end_r, vertical_direction
            ):
                if (
                    owned_pieces[intermediate_r, end_c] != 0
                    or enemy_pieces[intermediate_r, end_c] != 0
                    or obstacles[intermediate_r, end_c]
                ):
                    return False
        else:
            horizontal_direction = np.sign(end_c - start_c)
            for intermediate_c in range(
                start_c + horizontal_direction, end_c, horizontal_direction
            ):
                if (
                    owned_pieces[end_r, intermediate_c] != 0
                    or enemy_pieces[end_r, intermediate_c] != 0
                    or obstacles[end_r, intermediate_c]
                ):
                    return False
    else:
        if abs(end_r - start_r) > 1 or abs(end_c - start_c) > 1:
            print(
                "Non-scout piece type tried to move more than 1 space in a single turn"
            )
            return False
...
def _is_move_valid_by_1d_index(
    state: np.ndarray,
    player: INT_DTYPE_NP,
    action_index: INT_DTYPE_NP,
    action_size: INT_DTYPE_NP,
    ...
    if action_index == action_size - 1:
        ...
        valid_actions_mask = _get_valid_moves_as_1d_mask(
            state=state,
            player=player,
            action_size=action_size,
    ...
    result = _is_move_valid_by_position(
        state=state,
        player=player,
        start_r=start_r,
        start_c=start_c,
...
@jit(
    float32(INT_DTYPE_JIT[:, :, :], INT_DTYPE_JIT),
    nopython=True,
    fastmath=True,
    cache=True,
)
def _get_game_ended(state, player):
    if state[StateData.GAME_OVER.value]:
        ...
        return (
            game_winner * player
        )  # 1 or -1 depending on whether player or other player was winner
    else:
...
def _get_heuristic_rewards_from_move(
    state: np.ndarray,
    player: INT_DTYPE_NP,
    action_index: INT_DTYPE_NP,
    action_size: INT_DTYPE_NP,
...
def _get_next_state(
    ...
    if not _is_move_valid_by_1d_index(
        state=state,
        player=player,
        action_index=action_index,
        action_size=action_size,
    ...
    if dest_enemy_piece_type == SP.NOPIECE.value:
        ...
        if abs(end_r - start_r) > 1 or abs(end_c - start_c) > 1:
            ...
            owned_po_pieces[
                ...
                SP.SCOUT.value
            )  # player's long move distance revealed the piece as a Scout
        else:
            owned_po_pieces[
                end_r, end_c
            ] = moved_owned_po_piece_type  # player's piece type is not revealed by move

    else:
        # destination spot is contested, calculate winner of attack
        if (
            moved_owned_piece_type == SP.MINER.value
            and dest_enemy_piece_type == SP.BOMB.value
        ):
            moved_piece_wins_attack = True
        elif (
            moved_owned_piece_type == SP.SPY.value
            and dest_enemy_piece_type == SP.MARSHALL.value
        ):
            moved_piece_wins_attack = True
        ...
        if moved_piece_wins_attack:
            ...
            owned_po_pieces[
                end_r, end_c
            ] = moved_owned_piece_type  # player's piece type is revealed by attack

        if not moved_piece_wins_attack and not pieces_tied:
            enemy_po_pieces[
                end_r, end_c
            ] = dest_enemy_piece_type  # enemy piece won attack and has its type revealed
    ...
    if dest_enemy_piece_type != SP.NOPIECE.value:
        ...
        if not moved_piece_wins_attack:
            ...
            owned_captured_piece_layer = new_state[
                _get_player_captured_piece_layer(player, moved_owned_piece_type)
            ]
            owned_captured_piece_layer[end_r, end_c] = (
                owned_captured_piece_layer[end_r, end_c] + 1
            )

        if moved_piece_wins_attack or pieces_tied:
            # enemy's piece at the destination was captured
            enemy_captured_piece_layer = new_state[
                _get_player_captured_piece_layer(-player, dest_enemy_piece_type)
            ]
            enemy_captured_piece_layer[end_r, end_c] = (
                enemy_captured_piece_layer[end_r, end_c] + 1
            )
    ...
    if dest_enemy_piece_type == SP.NOPIECE.value:
        ...
        if old_recent_moves_map[end_r, end_c] == RecentMoves.JUST_CAME_FROM.value:
            if (
                old_recent_moves_map[start_r, start_c]
                == RecentMoves.JUST_ARRIVED_AND_NEXT_DOUBLE_BACK_IS_ILLEGAL.value
            ):
                new_recent_moves_map[
                    end_r, end_c
                ] = RecentMoves.JUST_ARRIVED_AND_CANT_DOUBLE_BACK.value
            else:
                new_recent_moves_map[
    ...
    max_turned_count_reached = (
        new_state[StateData.TURN_COUNT.value] >= new_state[StateData.MAX_TURNS.value]
    )
...
FULLY_OBSERVABLE_OBS_NUM_LAYERS = INT_DTYPE_NP(
    33
)  # Deprecated, use FULLY_OBSERVABLE_OBS_NUM_LAYERS_EXTENDED
...
def _get_fully_observable_observation(state, player, rows, columns):
    ...
    observation = np.empty(
        shape=(
            np.int64(rows),
            np.int64(columns),
            np.int64(FULLY_OBSERVABLE_OBS_NUM_LAYERS),
        ),
        dtype=np.float32,
    )
    ...
    observation[
        :, :, FullyObservableObsLayers.PLAYER_1_RECENT_MOVES.value
    ] = self_recent_moves
    observation[
        :, :, FullyObservableObsLayers.PLAYER_2_RECENT_MOVES.value
    ] = enemy_recent_moves

    observation[
        :, :, FullyObservableObsLayers.PLAYER_1_PO_PIECES.value
    ] = owned_po_pieces
    observation[
        :, :, FullyObservableObsLayers.PLAYER_2_PO_PIECES.value
    ] = enemy_po_pieces

    for state_layer, obs_layer in zip(
        range(p1_cap_start, p1_cap_end),
        range(
    ...
    observation[
        :, :, FullyObservableObsLayers.PLAYER_1_STILL_PIECES.value
    ] = owned_still_pieces
    observation[
        :, :, FullyObservableObsLayers.PLAYER_2_STILL_PIECES.value
    ] = enemy_still_pieces
...
class PartiallyObservableObsLayers(Enum):
    ...
    PLAYER_1_PIECES = INT_DTYPE_NP(
        0
    )  # actual piece values for current player's pieces, can't be SP.UNKNOWN
    PLAYER_1_PO_PIECES = INT_DTYPE_NP(
        1
    )  # piece values for current player's pieces, can be SP.UNKNOWN
    PLAYER_2_PO_PIECES = INT_DTYPE_NP(
        2
    )  # piece values for opponents pieces, can be SP.UNKNOWN
    OBSTACLES = INT_DTYPE_NP(3)

    PLAYER_1_RECENT_MOVES = INT_DTYPE_NP(
        4
    )  # only tracks the movement of the last moved piece to prevent oscillating a single piece in place forever
    PLAYER_2_RECENT_MOVES = INT_DTYPE_NP(5)

    PLAYER_1_CAPTURED_PIECE_RANGE_START = INT_DTYPE_NP(
        6
    )  # locations and counts of captured pieces, each layer tracks a different piece class
...
PARTIALLY_OBSERVABLE_OBS_NUM_LAYERS = INT_DTYPE_NP(
    32
)  # Deprecated, use PARTIALLY_OBSERVABLE_OBS_NUM_LAYERS_EXTENDED
...
def _get_partially_observable_observation(state, player, rows, columns):
    ...
    observation = np.empty(
        shape=(
            np.int64(rows),
            np.int64(columns),
            np.int64(PARTIALLY_OBSERVABLE_OBS_NUM_LAYERS),
        ),
        dtype=np.float32,
    )
    ...
    observation[
        :, :, PartiallyObservableObsLayers.PLAYER_1_PO_PIECES.value
    ] = owned_po_pieces
    observation[
        :, :, PartiallyObservableObsLayers.PLAYER_2_PO_PIECES.value
    ] = enemy_po_pieces
    observation[:, :, PartiallyObservableObsLayers.OBSTACLES.value] = obstacle_map
    observation[
        :, :, PartiallyObservableObsLayers.PLAYER_1_RECENT_MOVES.value
    ] = self_recent_moves
    observation[
        :, :, PartiallyObservableObsLayers.PLAYER_2_RECENT_MOVES.value
    ] = enemy_recent_moves
    ...
    observation[
        :, :, PartiallyObservableObsLayers.PLAYER_1_STILL_PIECES.value
    ] = owned_still_pieces
    observation[
        :, :, PartiallyObservableObsLayers.PLAYER_2_STILL_PIECES.value
    ] = enemy_still_pieces
...
class FullyObservableObsLayersExtendedChannels(Enum):
    PLAYER_1_PIECES_RANGE_START = INT_DTYPE_NP(
        0
    )  # piece values for current player's pieces, can't be SP.UNKNOWN
    PLAYER_1_PIECES_RANGE_END = INT_DTYPE_NP(12)

    PLAYER_2_PIECES_RANGE_START = INT_DTYPE_NP(
        12
    )  # piece values for opponent's pieces, can't be SP.UNKNOWN
    PLAYER_2_PIECES_RANGE_END = INT_DTYPE_NP(24)

    PLAYER_1_PO_PIECES_RANGE_START = INT_DTYPE_NP(
        24
    )  # piece values for current player's pieces, can be SP.UNKNOWN
    PLAYER_1_PO_PIECES_RANGE_END = INT_DTYPE_NP(37)

    PLAYER_2_PO_PIECES_RANGE_START = INT_DTYPE_NP(
        37
    )  # piece values for opponent's pieces, can be SP.UNKNOWN
...
def _get_fully_observable_observation_extended_channels(state, player, rows, columns):
    ...
    observation = np.empty(
        shape=(
            np.int64(rows),
            np.int64(columns),
            np.int64(FULLY_OBSERVABLE_OBS_NUM_LAYERS_EXTENDED),
        ),
        dtype=np.float32,
    )
    ...
    for piece_type, obs_layer in zip(
        range(1, 13),
        range(
        ...
        observation[:, :, obs_layer] = np.asarray(
            owned_pieces == piece_type, dtype=np.float32
        )
    ...
    for piece_type, obs_layer in zip(
        ...
        observation[:, :, obs_layer] = np.asarray(
            enemy_pieces == piece_type, dtype=np.float32
        )
    ...
    for piece_type, obs_layer in zip(
        ...
        observation[:, :, obs_layer] = np.asarray(
            owned_po_pieces == piece_type, dtype=np.float32
        )
    ...
    for piece_type, obs_layer in zip(
        ...
        observation[:, :, obs_layer] = np.asarray(
            enemy_po_pieces == piece_type, dtype=np.float32
        )

    observation[
        :, :, FullyObservableObsLayersExtendedChannels.OBSTACLES.value
    ] = obstacle_map
    observation[
        :, :, FullyObservableObsLayersExtendedChannels.PLAYER_1_RECENT_MOVES.value
    ] = self_recent_moves
    observation[
        :, :, FullyObservableObsLayersExtendedChannels.PLAYER_2_RECENT_MOVES.value
    ] = enemy_recent_moves
    ...
    observation[
        :, :, FullyObservableObsLayersExtendedChannels.PLAYER_1_STILL_PIECES.value
    ] = owned_still_pieces
    observation[
        :, :, FullyObservableObsLayersExtendedChannels.PLAYER_2_STILL_PIECES.value
    ] = enemy_still_pieces
...
def _get_partially_observable_observation_extended_channels(
    state, player, rows, columns
):
    state = _get_state_from_player_perspective(state=state, player=player)
    ...
    observation = np.empty(
        shape=(
            np.int64(rows),
            np.int64(columns),
            np.int64(PARTIALLY_OBSERVABLE_OBS_NUM_LAYERS_EXTENDED),
        ),
        dtype=np.float32,
    )
    ...
    observation[
        :, :, PartiallyObservableObsLayersExtendedChannels.OBSTACLES.value
    ] = obstacle_map
    observation[
        :, :, PartiallyObservableObsLayersExtendedChannels.PLAYER_1_RECENT_MOVES.value
    ] = self_recent_moves
    observation[
        :, :, PartiallyObservableObsLayersExtendedChannels.PLAYER_2_RECENT_MOVES.value
    ] = enemy_recent_moves
    ...
    observation[
        :, :, PartiallyObservableObsLayersExtendedChannels.PLAYER_1_STILL_PIECES.value
    ] = owned_still_pieces
    observation[
        :, :, PartiallyObservableObsLayersExtendedChannels.PLAYER_2_STILL_PIECES.value
    ] = enemy_still_pieces

    return observation


def _get_dict_of_valid_moves_by_position(
    state: np.ndarray,
    player,
    rows,
    columns,
    ...
    valid_moves_mask = _get_valid_moves_as_1d_mask(
        state=state,
