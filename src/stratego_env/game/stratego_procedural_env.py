...
from stratego_env.game.stratego_procedural_impl import (
    INT_DTYPE_NP, SP, StateLayers, _create_initial_state,
    _get_action_1d_index_from_player_perspective,
    _get_action_1d_index_from_positions,
    _get_action_1d_index_from_spatial_index,
    _get_action_positions_from_1d_index,
    _get_action_positions_from_player_perspective,
    _get_action_positions_from_spatial_index, _get_action_size,
    _get_action_spatial_index_from_1d_index,
    _get_action_spatial_index_from_positions,
    _get_dict_of_valid_moves_by_position, _get_fully_observable_observation,
    _get_fully_observable_observation_extended_channels, _get_game_ended,
    _get_game_result_is_invalid, _get_max_possible_actions_per_start_position,
    _get_next_state, _get_partially_observable_observation,
    _get_partially_observable_observation_extended_channels,
    _get_spatial_action_size, _get_state_from_player_perspective,
    _get_valid_moves_as_1d_mask, _get_valid_moves_as_spatial_mask,
    _is_move_valid_by_1d_index, _is_move_valid_by_position)
...
class StrategoProceduralEnv(object):
    ...
    def __init__(self, rows: int, columns: int):
        if rows < 3 or columns < 3:
            raise ValueError(
                "Both rows and columns have to be at least 3 (you passed rows: {} columns: {}).".format(
                    rows, columns
                )
            )
        ...
        self.spatial_action_size = _get_spatial_action_size(
            rows=self.rows, columns=self.columns
        )
        self._mpapsp = _get_max_possible_actions_per_start_position(
            rows=self.rows, columns=self.columns
        )
    ...
    def create_initial_state(
        ...
        if not np.array_equal(obstacle_map.shape, correct_shape):
            raise ValueError(
                "obstacle map needs to be of shape {}, was {}",
                correct_shape,
                obstacle_map.shape,
            )
        ...
        if not np.array_equal(player_1_initial_piece_map.shape, correct_shape):
            raise ValueError(
                "player_1_initial_piece_map map needs to be of shape {}, was {}",
                correct_shape,
        ...
        if not np.array_equal(player_2_initial_piece_map.shape, correct_shape):
            raise ValueError(
                "player_2_initial_piece_map map needs to be of shape {}, was {}",
                correct_shape,
    ...
    def get_action_1d_index_from_positions(self, start_r, start_c, end_r, end_c):
        return _get_action_1d_index_from_positions(
            ...
            start_r=INT_DTYPE_NP(start_r),
            start_c=INT_DTYPE_NP(start_c),
            end_r=INT_DTYPE_NP(end_r),
    ...
    def get_action_positions_from_1d_index(self, action_index):
        return _get_action_positions_from_1d_index(
            rows=self.rows,
            columns=self.columns,
    ...
    def get_valid_moves_as_1d_mask(
        self, state: np.ndarray, player, player_perspective=False
    ):
        if player_perspective and player == -1:
            state = _get_state_from_player_perspective(state=state, player=player)

        return _get_valid_moves_as_1d_mask(
            state=state,
    ...
    def get_dict_of_valid_moves_by_position(self, state: np.ndarray, player):
        return _get_dict_of_valid_moves_by_position(
            state=state,
            player=player,
            rows=self.rows,
            columns=self.columns,
    ...
    def is_move_valid_by_position(
        ...
        return _is_move_valid_by_position(
            state=state,
            player=INT_DTYPE_NP(player),
            start_r=INT_DTYPE_NP(start_r),
            start_c=INT_DTYPE_NP(start_c),
    ...
    def is_move_valid_by_1d_index(
        self, state: np.ndarray, player, action_index, allow_piece_oscillation=False
    ):
        return _is_move_valid_by_1d_index(
            state=state,
            player=INT_DTYPE_NP(player),
            action_index=INT_DTYPE_NP(action_index),
            action_size=self.action_size,
    ...
    def get_state_from_player_perspective(self, state: np.ndarray, player):
        return _get_state_from_player_perspective(
            state=np.asarray(state, dtype=INT_DTYPE_NP), player=INT_DTYPE_NP(player)
        )

    def get_action_positions_from_player_perspective(
        self, player, start_r, start_c, end_r, end_c
    ):
        return _get_action_positions_from_player_perspective(
            player=player,
            start_r=start_r,
            start_c=start_c,
            end_r=end_r,
    ...
    def get_action_1d_index_from_player_perspective(self, action_index, player):
        return _get_action_1d_index_from_player_perspective(
            action_index=INT_DTYPE_NP(action_index),
            player=INT_DTYPE_NP(player),
            action_size=self.action_size,
            rows=self.rows,
    ...
    def get_action_positions_from_spatial_index(self, spatial_index: np.ndarray):
        return _get_action_positions_from_spatial_index(
            rows=self.rows, columns=self.columns, spatial_index=spatial_index
        )

    def get_valid_moves_as_spatial_mask(self, state, player):
        return _get_valid_moves_as_spatial_mask(
            state=state, player=INT_DTYPE_NP(player)
        )
    ...
    def get_action_spatial_index_from_1d_index(self, action_index):
        return _get_action_spatial_index_from_1d_index(
            rows=self.rows,
            columns=self.columns,
            action_index=INT_DTYPE_NP(action_index),
            action_size=self.action_size,
    ...
    def get_fully_observable_observation(self, state: np.ndarray, player):
        return _get_fully_observable_observation(
            state=state,
            player=INT_DTYPE_NP(player),
            rows=self.rows,
            columns=self.columns,
    ...
    def get_partially_observable_observation(self, state: np.ndarray, player):
        return _get_partially_observable_observation(
            state=state,
            player=INT_DTYPE_NP(player),
            rows=self.rows,
            columns=self.columns,
    ...
    def get_fully_observable_observation_extended_channels(
        self, state: np.ndarray, player
    ):
        return _get_fully_observable_observation_extended_channels(
            state=state,
            player=INT_DTYPE_NP(player),
            rows=self.rows,
            columns=self.columns,
    ...
    def get_partially_observable_observation_extended_channels(
        self, state: np.ndarray, player
    ):
        return _get_partially_observable_observation_extended_channels(
            state=state,
            player=INT_DTYPE_NP(player),
            rows=self.rows,
            columns=self.columns,
    ...
    def get_serializable_string_for_fully_observable_state(self, state: np.ndarray):
        return dumps(
            _get_fully_observable_observation(
                state=state,
                player=INT_DTYPE_NP(1),
                rows=self.rows,
                columns=self.columns,
            )
        )
    ...
    def get_serializable_string_for_partially_observable_state(self, state: np.ndarray):
        return dumps(
            _get_partially_observable_observation(
                state=state,
                player=INT_DTYPE_NP(1),
                rows=self.rows,
                columns=self.columns,
    ...
    def print_board_to_console(
        self, state, partially_observable=False, hide_still_piece_markers=True
    ):
        p2_state_layer = (
            StateLayers.PLAYER_2_PO_PIECES.value
            if partially_observable
            else StateLayers.PLAYER_2_PIECES.value
        )
